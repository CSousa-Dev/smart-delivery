...
Scenario: Atualizar dados básicos de um usuário existente
  ...
  When o administrador solicita a atualização dos dados básicos
...
Scenario: Atualizar apenas parte dos dados
  Given que existe um usuário previamente criado
  When o administrador atualiza apenas um dos campos permitidos
...
Scenario: Tentar atualizar com payload vazio
  Given que existe um usuário previamente criado
  When o administrador envia uma atualização sem campos válidos
...
quero **impedir que a atualização de identidade afete autenticação ou autorização**,

para garantir separação clara de responsabilidades dentro do módulo Auth.
...
```gherkin
Scenario: Atualização não afeta autenticação nem autorização
  ...
  And nenhum acesso ou permissão operacional deve ser concedido ou revogado
...
- **FR-006**: A atualização de um usuário **NÃO DEVE** conceder, revogar ou alterar permissões, roles ou acessos operacionais.
