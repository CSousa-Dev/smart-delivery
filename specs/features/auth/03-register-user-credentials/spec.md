...
Scenario: Registrar credenciais para um usuário existente
  ...
  And que o usuário está com status ACTIVE
...
Scenario: Tentar registrar credenciais para usuário INACTIVE
  Given que existe um usuário com status INACTIVE
  When é solicitado o registro de credenciais
  Then o registro deve ser rejeitado
  And o sistema deve informar que o usuário está inativo
...
- **FR-007**: O sistema **NÃO DEVE** permitir registro ou atualização de credenciais para usuários com status `INACTIVE`.
- **FR-008**: O registro ou atualização de credenciais **NÃO DEVE** criar sessão, token ou estado autenticado.
...
- **SC-001**: 100% das credenciais registradas estão associadas a usuários válidos e ativos.
