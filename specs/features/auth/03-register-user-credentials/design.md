...
A capability **Register User Credentials** permite **registrar ou atualizar credenciais de autenticação** associadas a uma identidade existente e ativa.
...
sequenceDiagram
    ...
    Client->>Controller: POST /users/:id/credentials
    ...
    Database-->>UserRepository: User (ACTIVE)
...
| Usuário deve estar `ACTIVE` | Validado no Application |
...
| --- | --- | --- | --- | --- |
| POST | `/users/:id/credentials` | Registrar | 201 | 400, 404, 409 |
| PUT | `/users/:id/credentials/:method` | Atualizar | 200 | 400, 404 |
...
flowchart LR
    ...
    INACTIVE[UserInactiveError] --> H409[409]
...
- Nenhuma dependência com Permissions Provider
