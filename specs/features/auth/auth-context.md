# Auth Context - Security & Token Strategy
...
Este documento se aplica exclusivamente ao **contexto Auth** e às seguintes capabilities:

- Register User Credentials
- Authenticate User
- Authorize Action (token verification only)
...
- Ser tratado como **opaco** fora do Auth

O token **NÃO DEVE**:

- Conter permissões, roles ou grants
- Conter semântica de domínio (tenant, BU, etc.)
- Ser interpretado fora do Auth

---

### Usage Constraints

- Emissão do token ocorre **somente** no Auth
- Verificação ocorre na Infrastructure (Auth ou consumidores)
...
- Tokens **não carregam autorização**
- Autorização é sempre delegada via `PermissionsProvider`
- Token serve apenas para **identificação da identidade**
- Mudanças de permissão exigem **Invalidate Authorization**
