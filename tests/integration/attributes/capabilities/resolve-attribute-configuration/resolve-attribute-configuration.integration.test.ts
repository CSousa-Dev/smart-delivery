...
describeIf('Capability Resolve Attribute Configuration – [CAP-007]', () => {
  ...
  it('should resolve attributes for vertical – [SCN-001]', async () => {
    ...
    const service = new ResolveAttributeConfigurationService(
    ...
    const output = await service.list({ verticalId: vertical.id });

    expect(output.items).toHaveLength(1);
    expect(output.items[0]?.code).toBe('SIZE');
  });
});
